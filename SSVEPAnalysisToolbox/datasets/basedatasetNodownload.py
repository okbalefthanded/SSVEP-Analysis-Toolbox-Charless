# -*- coding: utf-8 -*-
"""
Modified Base class of SSVEP datasets to remove download process
"""
...
class BaseDatasetNoDownload(metaclass=abc.ABCMeta):
    """
    BaseDataset
    """
    def __init__(self, 
...
# ! Update here
    def get_data_single_trial(self,
        ...
        #     eeg_trial.append(eeg_total[:,:,start_t_idx:end_t_idx])
        #     start_t_idx += sig_len
    ...
    def get_data_trial(self,
                       ...
                       t_latency : float):
        # ! TODO by handsome Charless
        # nested_list = [self.get_data_single_trial(sub_data, block_idx, trial_idx, channels, sig_len, t_latency) for trial_idx in trials]
    ...
    def get_label_trial(self,
        ...
        """
        # ! TODO by handsome Charless
        # nested_list = [self.get_label_single_trial(sub_idx, block_idx, trial_idx, sig_len) for trial_idx in trials]
        # flattened_list = [item for sublist in nested_list for item in sublist]

        return [self.get_label_single_trial(sub_idx, block_idx, trial_idx, sig_len) for trial_idx in trials]
    ...
    def get_phase(self,
        ...
        if sig_len is None:
            sig_len = self.trial_len
        phase = np.zeros((len(self.subjects), self.block_num, self.trial_num, len(self.channels))) # subj * block_num * stimulus_num * ch_num
        if remove_pre_and_latency:
            sig_len = sig_len - self.t_prestim - t_latency
        if remove_break:
            sig_len -= self.t_break
        ...
        for sub_idx in range(len(self.subjects)):
            for block_idx in range(self.block_num):
                ...
                for trial_idx, X_single_trial in enumerate(X_all_trials):
                    ...
                    for ch_idx in range(len(self.channels)):
                        X_fft = X_single_trial[filter_bank_idx, ch_idx:(ch_idx+1), :]
                        phase[sub_idx, block_idx, trial_idx, ch_idx] = freqs_phase(X_fft, self.stim_info['freqs'][Y_fft], self.stim_info['phases'][Y_fft],
                                                                                srate, 
                                                                                detrend_flag = detrend_flag,
                                                                                NFFT = NFFT)
                        if remove_target_phase:
